						...
						<table class="table bootstrap-datatable countries" id="memberstable">
							...
							%>
								<tr>
									...
									<form action="<spring:url value="/group/changestorage"/>" method="post">
										<td>
											<select id="storage" class="form-control m-bot15" name="storage">
    											<option value="10" <%=  fgu.getStorage()==10?"selected":"" %>> 10 GB</option>
    											<option value="20" <%=  fgu.getStorage()==20?"selected":"" %>> 20 GB</option>
    											<option value="30" <%=  fgu.getStorage()==30?"selected":"" %>> 30 GB</option>
    											<option value="40" <%=  fgu.getStorage()==40?"selected":"" %>> 40 GB</option>
    											<option value="50" <%=  fgu.getStorage()==50?"selected":"" %>> 50 GB</option>
    											<option value="60" <%=  fgu.getStorage()==60?"selected":"" %>> 60 GB</option>
    											<option value="70" <%=  fgu.getStorage()==70?"selected":"" %>> 70 GB</option>
    											<option value="80" <%=  fgu.getStorage()==80?"selected":"" %>> 80 GB</option>
    											<option value="90" <%=  fgu.getStorage()==90?"selected":"" %>> 90 GB</option>
    											<option value="100" <%= fgu.getStorage()==100?"selected":"" %>> 100 GB</option>
											</select>
										</td>
										<td>
                      						<input type="hidden"  name="fguid" value="<%= fgu.getFguid() %>">
                      						<div class="btn-group">
                        						<input  type="submit" name="button" value="change"   class="btn btn-primary btn-sm" />
                    				...
                    				<td>
										<form action="<spring:url value="/group/activemember"/>" method="post">
                      						<input type="hidden"  name="fguid" value="<%= fgu.getFguid() %>">
                      						<div class="btn-group">
                      							<% if(fgu.getState()==1){%>
                        							<input  type="submit" name="button" value="deactivate" class="btn btn-danger btn-sm" />
                        						<%} else{ %>
