...
<script>
  function delete_job(id) {
	 ...
	 $dialog.dialog('open');
	 $dialog.html('<p><span class="ui-icon ui-icon-alert" style="float:left; margin:12px 12px 20px 0;"></span>This Job will be permanently Canceled and cannot be recovered. Are you sure?</p>');
  }
</script>
...
if(s!=null && s.equals("true")){%>
	<div class="alert alert-success alert-dismissible fade in">
    	...
    	<strong>Success!</strong> Job has been sent successfully.
    </div>
<%}%>
...
<% if(s!=null && s.equals("false")){%>
	<div class="alert alert-warning alert-dismissible fade in">
    	<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
    	<strong>Warning!</strong> job submission failed!
    </div>
<%}%> --%>
				...
				<div class="row">
					<div class="col-lg-12">
						<h3 class="page-header">
							<i class="fa fa-laptop"></i> Jobs List
						</h3>
						...
						<ol class="breadcrumb">
							...
							<li><i class="icon_document_alt"></i>Job</li>
              				<li><i class="fa fa-files-o"></i>Jobs List</li>
						</ol>
					</div>
				</div>
				<!--/.row-->
							...
							<div class="panel-body">
								<div class="row">
									<div class="btn-group btn-group-justified">
											 <a class="btn btn-info active" style="background-color: white; color: #000000;" disabled>Queued</a>
											 <a class="btn btn-warning" style="background-color: #ffff80; color: #000000;" disabled>Running</a>
											 <a class="btn btn-success" style="background-color: #ccffcc; color: #000000;" disabled>Concluded</a>
									</div>
								...
								<table class="table bootstrap-datatable countries" id="jobstable">
									...
									%>
										...
										<form  action="<spring:url value="/job/editselected"/>" method="get"> 
											<td><%= row%></td>
											<td><%= j.getJobid() %> </td>
											<td><%= username %> </td>
											<td><%= j.getJname() %></td>
											<td><input type="hidden"  name="appname" value="<%= j.getAppname() %>"><%= j.getAppname() %></td>
											<td><%= j.getWalltime() %></td>
											<td><%= Util.getTime(j.getCputime())%></td>
                    						...
                    						<form id="myform<%=row%>" action="/job/deleteselected" method="post">
                    						<td>
                    							<input type="hidden"  name="jobid" value="<%= j.getJobid() %>" >
                    							<% if(!j.getState().equals("C")){%>
                        							<input  type="button" name="button" value="cancel" class="btn btn-danger" onclick='delete_job("myform<%=row%>")'/>
                        						<%} %>
                    						</td>
											</form>
										</tr>
